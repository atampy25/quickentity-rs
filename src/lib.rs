...
use patch_structs::{
    ArrayPatchOperation, Patch, PatchOperation, PropertyOverrideConnection,
    SetPlatformSpecificPropertyValue, SetPropertyValue, SubEntityOperation,
};
use qn_structs::{
    ...
    pool.install(|| {
                ...
                entities: factory
                    ...
                    .map(
                        |(index, sub_entity_factory)| -> Result<(String, SubEntity)> {
                            let sub_entity_blueprint = blueprint.sub_entities.get(index).context(
                                "Factory entity had no equivalent by index in blueprint",
                            )?;
...

        {
            ...
            entity.extra_factory_dependencies = factory_meta
                ...
                .map(|x| match x {
                    RpkgResourceReference { hash, flag } if flag == "1F" => {
                        Dependency::Short(hash.to_string())
                    }
                ...
                entity
                    ...
                    .map(|x| {
                        Ok(match x {
                            ...
                            Dependency::Full(DependencyWithFlag { resource, flag }) => {
                                RpkgResourceReference {
                                    hash: resource.parse()?,
                                    flag: flag.to_owned(),
                                }
                            ...
                            .map(|(property, overridden)| {
                                Ok(resourcelib::PropertyOverride {
                                    ...
                                    property_value: resourcelib::Property {
                                        n_property_id: convert_string_property_name_to_rt_id(
                                            property,
                                        )?,
                                        value: resourcelib::PropertyValue {
